...
const width = 500,
  height = 300,
  data = [50, 90, 120, 160, 200],             /* 假数据 */
  linear = d3
    .scaleLinear()                            /* 比例映射，假设固定一块区域（宽：500， 高：500），然而数据的值超过了500，总不能超过整个画布，所以利用 d3.scaleLinear 来映射 */
    .domain([0, d3.max(data)])                /* domain 域的大小从 0 到给定数据的最大值 */ 
    .range([0, width - 30]),                  /* range 代表画布从 0 到画布宽度（有边框、间隙需减去） */
